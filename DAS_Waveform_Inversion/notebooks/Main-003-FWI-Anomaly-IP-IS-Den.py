...
from FWI_ops import FWI_obscalc, FWI_IP_IS_Den
...
parser.add_argument('--exp_name', type=str, default='/scratch/users/haipeng/Anomaly-IP-IS-Den')
...
if generate_data:
    ...
    # load true model
    vp_true  = np.loadtxt('./Models/Anomaly_P-WAVE_VELOCITY_101_201_IP_IS_Den.txt').astype('float32')
    vs_true  = np.loadtxt('./Models/Anomaly_S-WAVE_VELOCITY_101_201_IP_IS_Den.txt').astype('float32')
    den_true = np.loadtxt('./Models/Anomaly_DENSITY_101_201_IP_IS_Den.txt').astype('float32')
...
## Full Waveform Inversion
    ...
    # load initial model
    vp_init  = np.loadtxt('./Models/Anomaly_P-WAVE_VELOCITY_101_201_IP_IS_Den_init.txt').astype('float32')
    vs_init  = np.loadtxt('./Models/Anomaly_S-WAVE_VELOCITY_101_201_IP_IS_Den_init.txt').astype('float32')
    den_init = np.loadtxt('./Models/Anomaly_DENSITY_101_201_IP_IS_Den_init.txt').astype('float32')

    print('IP-IS-Den scheme')
    vp_init = vp_init / 1e3
    vs_init = vs_init / 1e3
    IP_init = vp_init * den_init 
    IS_init = vs_init * den_init

    th_IP_inv = torch.tensor(IP_init, dtype=torch.float32, requires_grad=True)
    th_IS_inv = torch.tensor(IS_init, dtype=torch.float32, requires_grad=True)
    ...
    fwi = FWI_IP_IS_Den(th_IP_inv, th_IS_inv, th_den_inv, th_Stf, opt, Mask=th_mask, IP_bounds=IP_bounds, \
            IS_bounds=IS_bounds, Den_bounds=Den_bounds)
    ...
    def save_prog(x):
        ...
        sio.savemat(result_dir_name + '/IP'      + str(__iter) + '.mat', {'IP':fwi.IP.cpu().detach().numpy()})
        sio.savemat(result_dir_name + '/IS'      + str(__iter) + '.mat', {'IS':fwi.IS.cpu().detach().numpy()})
        sio.savemat(result_dir_name + '/Den'     + str(__iter) + '.mat', {'Den':fwi.Den.cpu().detach().numpy()})
        sio.savemat(result_dir_name + '/grad_IP' + str(__iter) + '.mat', {'grad_IP':fwi.IP.grad.cpu().detach().numpy()})
        sio.savemat(result_dir_name + '/grad_IS' + str(__iter) + '.mat', {'grad_IS':fwi.IS.grad.cpu().detach().numpy()})
