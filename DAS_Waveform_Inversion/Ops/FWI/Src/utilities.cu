...
__global__ void add_source(float *d_szz, float *d_sxx, float amp, int nz,
  ...
  if (isFor) {
    if (gidz == 0  && gidx == 0 ) {
      int idz = 0;
      int idx = 0;
      d_szz[(z_loc + idz) + nz * (x_loc + idx)] += scale * amp * dt;
      // crosswell borehole source (can be modified) assume cp/cs = sqrt(3.0)
      d_sxx[(z_loc + idz) + nz * (x_loc + idx)] += scale * amp * dt;
    } else {
      return;
    }
  } else {
...
// EXX recording and injection
__global__ void recording_exx(float *d_vx, float *d_vz, int nz, float *d_data,
...
return;
  ...
  d_data[(iRec) * (nSteps) + (it)] = d_vx[d_z_rec[iRec] + d_x_rec[iRec] * nz] - d_vx[d_z_rec[iRec] + (d_x_rec[iRec] -1) * nz];
  // does not divide by dx
}


__global__ void res_injection_exx(float *d_vx_adj, float *d_vz_adj, int nz,
...
return;
  ...
  d_vx_adj[d_z_rec[iRec] + nz * d_x_rec[iRec]]     += d_res[(iRec) * (nSteps) + (it)];
  d_vx_adj[d_z_rec[iRec] + nz * (d_x_rec[iRec]-1)] -= d_res[(iRec) * (nSteps) + (it)];
}



// EZZ recording and injection
__global__ void recording_ezz(float *d_vx, float *d_vz, int nz, float *d_data,
  int iShot, int it, int nSteps, int nrec, int *d_z_rec,
  int *d_x_rec, double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
if (iRec >= nrec) {
return;
  ...
  d_data[(iRec) * (nSteps) + (it)] = d_vz[d_z_rec[iRec] + d_x_rec[iRec] * nz] - d_vz[d_z_rec[iRec] -1 + d_x_rec[iRec] * nz];
}


__global__ void res_injection_ezz(float *d_vx_adj, float *d_vz_adj, int nz,
  float *d_res, int it, float dt, int nSteps,
  int nrec, int *d_z_rec, int *d_x_rec,
  double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
...
return;
  ...
  d_vz_adj[d_z_rec[iRec] + nz * d_x_rec[iRec]]    += d_res[(iRec) * (nSteps) + (it)];
  d_vz_adj[d_z_rec[iRec] -1 + nz * d_x_rec[iRec]] -= d_res[(iRec) * (nSteps) + (it)];
}


// Vx recording and injection
__global__ void recording_vx(float *d_vx, float *d_vz, int nz, float *d_data,
  int iShot, int it, int nSteps, int nrec, int *d_z_rec,
  int *d_x_rec, double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
if (iRec >= nrec) {
return;
  ...
  d_data[(iRec) * (nSteps) + (it)] = d_vx[d_z_rec[iRec] + d_x_rec[iRec] * nz];
}


__global__ void res_injection_vx(float *d_vx_adj, float *d_vz_adj, int nz,
  float *d_res, int it, float dt, int nSteps,
  int nrec, int *d_z_rec, int *d_x_rec,
  double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
...
// Vz recording and injection
__global__ void recording_vz(float *d_vx, float *d_vz, int nz, float *d_data,
  int iShot, int it, int nSteps, int nrec, int *d_z_rec,
  int *d_x_rec, double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
if (iRec >= nrec) {
return;
  ...
  d_data[(iRec) * (nSteps) + (it)] = d_vz[d_z_rec[iRec] + d_x_rec[iRec] * nz];
}


__global__ void res_injection_vz(float *d_vx_adj, float *d_vz_adj, int nz,
  float *d_res, int it, float dt, int nSteps,
  int nrec, int *d_z_rec, int *d_x_rec,
  double *d_rxz) {
int iRec = threadIdx.x + blockDim.x * blockIdx.x;
...
// Pressure recording and injection
__global__ void recording(float *d_szz, float *d_sxx, int nz, float *d_data,
  ...
  d_data[(iRec) * (nSteps) + (it)] =
      ...
      d_sxx[d_z_rec[iRec] + d_x_rec[iRec] * nz];
}
