...
std::vector<torch::Tensor> fwi_forward(const torch::Tensor &th_Lambda,
  ...
  cufd(misfit_ptr, nullptr, nullptr, nullptr, nullptr, Lambda, Mu, Den, stf, 0,
       gpu_id, group_size, shot_ids, para_fname);
...
void fwi_obscalc(const torch::Tensor &th_Lambda, 
                 const torch::Tensor &th_Mu,
                 const torch::Tensor &th_Den, 
                 const torch::Tensor &th_stf,
...
#pragma omp parallel for num_threads(ngpu)
  for (int i = 0; i < ngpu; i++) {
    ...
    auto th_sub_shot_ids = th_shot_ids.narrow(0, startBar, endBar - startBar);
    auto shot_ids = th_sub_shot_ids.data_ptr<int>();
    cufd(nullptr, nullptr, nullptr, nullptr, nullptr, Lambda, Mu, Den, stf, 2,
         i, th_sub_shot_ids.size(0), shot_ids, para_fname);
  }
}
